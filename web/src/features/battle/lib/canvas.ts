...
function drawFlyingCap(
  ...
  const spin =
    (now / (isStormCap ? 58 : 95) + hashString(bullet.id)) % (Math.PI * 2)
  const arcTilt = Math.atan2(bullet.vy || 0, bullet.vx || 1) * 0.2

  ctx.save()
  let shadowColor = 'rgba(34, 197, 94, 0.65)'
  if (heavyScene) {
    shadowColor = 'transparent'
  } else if (isStormCap) {
    shadowColor = 'rgba(250, 204, 21, 0.82)'
  }
...
function drawPlayer(
  ...
  ctx.shadowColor = hasCapStorm
    ...
    : 'rgba(34, 197, 94, 0.24)'
  let shadowBlur = 0
  if (player.alive && !heavyScene) {
    shadowBlur = hasCapStorm ? 24 : 16
  }
