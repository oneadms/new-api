...
function StatusTimeline(props: {
  ...
  const { t } = useTranslation()
  const buckets = props.buckets.map((status, slot) => ({
    key: `${props.label}-${slot}`,
    status,
  }))
  ...
  return (
    <div className='flex min-w-0 flex-col gap-1.5'>
      ...
      >
        {buckets.map((bucket) => {
          const meta = getMonitorStatusMeta(bucket.status)
          return (
            <span
              key={bucket.key}
...
export function Status() {
  ...
  if (statusQuery.isError) {
    emptyStateTitle = t('Unable to load service status')
    emptyStateDescription = t('Please refresh and try again.')
  } else if (!groups.length) {
    emptyStateTitle = t('No uptime monitoring configured')
