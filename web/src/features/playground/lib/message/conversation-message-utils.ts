...
export function appendUserMessagePair(
  messages: Message[],
  content: string,
  options: { mode?: PlaygroundMode; imageReferenceUrls?: string[] } = {}
): Message[] {
  const submittedAt = Date.now()

  return [
    ...messages,
    createUserMessage(content, submittedAt, options),
    createLoadingAssistantMessage(submittedAt, { mode: options.mode }),
  ]
}
...
export function createRegeneratedMessages(
  ...
  if (messages[messageIndex].from === MESSAGE_ROLES.USER) {
    return [
      ...
      createLoadingAssistantMessage(Date.now(), {
  ...
  return {
    messages: [
      ...
      createLoadingAssistantMessage(submittedAt, {
        mode: updatedMessages[messageIndex].mode,
      }),
    ],
    shouldSend: true,
  }
