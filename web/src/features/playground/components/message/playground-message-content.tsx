...
function getGeneratedImageSrc(image: { url?: string; b64_json?: string }) {
...
export function PlaygroundMessageContent({
  ...
  const generatedImages = (message.images ?? []).flatMap((image, index) => {
    const src = getGeneratedImageSrc(image)
    return src ? [{ key: `${message.key}-image-${index}`, src }] : []
  })
  ...
  return (
    ...
    >
      ...
      {!isError && (showMessageContent || hasGeneratedImages) && (
        <>
          {isSourceVisible ? (
            <CodeBlock
              code={versionContent}
                ...
                <div className='mt-2 grid w-full gap-3 sm:grid-cols-2'>
                  {generatedImages.map((image) => (
                    <img
                      ...
                      className='border-border bg-muted aspect-square w-full rounded-lg border object-contain'
                      key={image.key}
                      src={image.src}
                    />
                  ))}
