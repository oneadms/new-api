...
func validateContentOpacity(value string) error {
	opacity, err := strconv.Atoi(strings.TrimSpace(value))
	if err != nil {
		return fmt.Errorf("透明度必须是 0 到 100 之间的整数")
	}
...
func UpdateOption(c *gin.Context) {
	...
	case "BackgroundImage":
		err = validateOptionalHTTPURL(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": err.Error(),
	...
	case "ActualModelRatio":
		err = ratio_setting.UpdateActualModelRatioByJSONString(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "实际模型倍率设置失败: " + err.Error(),
			})
			return
		}
	case "ImageRatio":
