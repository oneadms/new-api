...
func GenerateAccessToken(c *gin.Context) {
	...
	if err := model.DB.Model(&model.User{}).Where("access_token = ?", key).Count(&duplicateCount).Error; err != nil {
		common.ApiError(c, err)
		return
	}
	if duplicateCount != 0 {
...
func GetUserModels(c *gin.Context) {
	...
	groups, err := service.ResolveUserUsableGroups(c, user.Group)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	case group == "auto":
		if _, ok := groups[group]; ok {
			groupsToQuery = service.GetUserAutoGroupFromUsableGroups(groups)
		}
	default:
		if _, ok := groups[group]; ok {
