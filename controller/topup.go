...
func calculateTopUpCreditAmount(amount int64) (int64, error) {
	if amount <= 0 {
		return 0, fmt.Errorf("充值数量必须大于 0")
	}
...
func validateUserTopUpCreditAmount(userId int, amount int64) error {
	quota, err := calculateTopUpCreditAmount(amount)
	if err != nil {
		return err
	}
	return model.ValidateUserQuotaCredit(userId, quota)
}
