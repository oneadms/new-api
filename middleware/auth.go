...
// WebSocketUserAuth authenticates browser WebSocket requests with the same
// short-lived dashboard access token used by normal API calls. Browsers cannot
// set an Authorization header for WebSocket handshakes, so the frontend sends
// the token in a dedicated Sec-WebSocket-Protocol entry instead of the URL.
func WebSocketUserAuth(protocolPrefix string) func(c *gin.Context) {
	return func(c *gin.Context) {
		if _, ok := authorizationToken(c.GetHeader("Authorization")); !ok {
			for _, protocol := range strings.Split(c.GetHeader("Sec-WebSocket-Protocol"), ",") {
				...
				if token, found := strings.CutPrefix(protocol, protocolPrefix); found && token != "" {
...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		userUsableGroups, err := service.ResolveUserUsableGroups(c, userGroup)
		if err != nil {
			common.SysLog(fmt.Sprintf("TokenAuth resolve usable groups error for user %d: %v", token.UserId, err))
			abortWithOpenAiMessage(c, http.StatusInternalServerError,
				common.TranslateMessage(c, i18n.MsgDatabaseError))
			return
		}
